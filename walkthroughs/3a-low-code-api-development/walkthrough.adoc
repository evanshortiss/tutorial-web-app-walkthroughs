...
.. In the *Where should Fuse Online Demo post?* field select *low-code-demo* and click *Allow*.
+
...
. Edit the *Description* that's found in the *Design* tab on the right in the *INFO* section:
+
----
My greeting API
----
...
. In the *Choose to create a REST Resource with the Data Type* section verify *No Resource* is selected.
...
+
----
Greeting Integration {user-sanitized-username}
----
...
. Click the *name* field under the *body*, then click the *body* field in the *Target* panel. This maps the *name* value from the incoming HTTP request *body* to the *body* of the HTTP response sent to a client.
...
. From the *Dashboard*, select the *Products* tab.
. Click *New Product* in the *Products* tab.
. Select the *Import from OpenShift* option.
If this option is not enabled, click the *Authenticate to enable this option* link and choose *Allow selected permissions*.

. Verify that the *fuse* option is selected from the *Namespace* list.
. Verify that the *i-greeting-api-{user-sanitized-username}* is selected from the *Name* list.
...
.. Choose *i-greeting-integration-{user-sanitized-username}* from the top navigation menu to view the {3scale-name} homepage.

+
NOTE: If the *i-greeting-integration-{user-sanitized-username}* is not listed in the top navigation menu that means it is still being imported. Wait a while, refresh the page, and try again.

.. From the *Overview* screen, select *Integration > Settings* from the left menu.
...
.. Choose *Integration > Configuration*. Note that there is a warning icon beside this link that suggests promoting the Configuration.
...
.. Select the *low-code* Plan in the *Application plan* menu under the *i-greeting-integration-{user-sanitized-username}*.
.. Enter the following for *Name* and *Description*:
+
[subs="attributes+"]
----
low-code-app-{user-sanitized-username}
----
...
. Return to the *Integration > Configuration* screen for your *i-greeting-integration-{user-sanitized-username}* in 3scale.
