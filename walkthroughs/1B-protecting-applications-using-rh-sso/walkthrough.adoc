...
// TODO: Verify this... In my case, the user `evals03` was already logged in (the name in top-right corner), I assume because there was a session in RH-SSO. Or maybe instruct here to open in the private browser session (like in the verification step).
