...
. Log into link:{openshift-host}/console/{walkthrough-namespace}/overview[OpenShift Namespace, window="_blank"] for this walkthrough.
. Select *Addresses > Pods* from the left hand menu.
. Click the Pod that has the name *orders-db* followed by an ID
. On the Pod details screen select the *Terminal* tab and issue.
. Login to the *orders* database using the command `psql -U {user-username} -W -d orders`.
