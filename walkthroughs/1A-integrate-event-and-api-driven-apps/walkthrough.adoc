...
* An Order Entry System, built with Node.js, that provides a RESTful API and UI to create order messages.
* An Order Management System RESTful API, built with Red Hat Fuse, for querying and creating orders in an underlying PostgreSQL.
* An Order Management System UI, built with Node.js, for visualising orders in the PostgreSQL via the Fuse RESTful API.
...
To store Orders a table must be created in the PostgreSQL that was provisioned.
...
. Click the Pod that has the name *orders-db* followed by an ID.
. On the Pod details screen select the *Terminal* tab.
. Log in to the *orders* database using the command `psql -U {user-username} -W -d orders`.
...
----
.. Enter forward slash, i.e `/`, in the *Base path* field
...
// TODO: currently the app has the login screen, either remove the screen in the app and allow free access, or instruct the user here to enter a username ([A-Za-z]{1,15}) and click *Log in*.
